...
//!
//! The point of the harness is that a number carries the machine it came from
//! and the versions it compared. That is a property of this code rather than of
...
//!
//! Order matters for that last part. `render` checks the leg, then the value,
//! then the machine, then the comparands, so a fixture that is blank in two
//! places proves only the earlier check. Every fixture below is complete apart
//! from the one field its test is about.

use indexwerk_harness::{Compared, Machine, Measurement, Outcome, Refused, dispatch, render};
...
#[test]
fn a_complete_measurement_renders_and_carries_both() {
    let rendered = render(&a_machine(), &a_measurement()).expect("a complete measurement renders");
    assert!(
        rendered.contains("machine: 16 cores, 64 GiB"),
        "the machine description has to reach the output, got:\n{rendered}"
    );
    assert!(
        rendered.contains("indexwerk 0.0.0"),
        "every comparand and its version has to reach the output, got:\n{rendered}"
    );
    assert!(
        rendered.contains("result: a speedup of 11.4 at 16 threads"),
        "the value has to reach the output, got:\n{rendered}"
    );
}
