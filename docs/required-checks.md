...
| `.github/workflows/` | zizmor, on `Audit workflows (zizmor)`; the Unicode guard | no code analysis engine, which is a deviation rather than a gap: the target gate of #37 runs an engine over its own action definitions, and `docs/quality-parity.md` records the audit leg as this board's answer to that element |
